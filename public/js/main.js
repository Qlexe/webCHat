...
const url = "https://9lwd59-3000.csb.app/messages/";
...
function handler(e) {
  if (e.code === "Enter" || e.type == "click") {
    if (!localStorage.getItem("user")) {
      ...
      fetch(url, requestOptions).then((data) => console.log(data)); // Робимо щось з даними
  ...
  if (localStorage.getItem("user")) {
    fetch(url)
      ...
      .then((data) => {
        ...
        if (data.length > 1)
          document.querySelector("#start_message").hidden = true;
